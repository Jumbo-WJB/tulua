...
def make_cookie(host,name, value):
...
def add_cookies(host,cookieFile):
	global opener ,cj
	fp=open(cookieFile,'r')
	cookies=fp.read().split(';')
	for c in cookies:
		kv=c.split('=')
...
def main():
	...
	try:
		opts, args = getopt.getopt(sys.argv[1:], "vhsd:a:u:l:r:q:b:c:", ["help"])
	...
	try:
		for o, a in opts:
			...
			if o == "-s":
				start_p_thread=True
			if o == "-c":
				add_cookies(url.split('/')[1].split('/')[0],a)
